...
def triad(time):
  ...
  timeline.add(time + 0.075 + math.sin(increment), Hit(Note(notes[5]), duration))
      
def strum(time, offset = 0.075):
  for j, note in enumerate(notes):
      timeline.add(time + offset * math.cos(j) + math.sin(increment), Hit(Note(note), duration))

time += sixteenth_note + random.uniform(0.5, 1.5)
...
for i in range(iterations):
  ...
  for j, note in enumerate(notes):
    timeline.add(time, Hit(Note(note), duration + math.sin(time)))
    timeline.add(time + 0.75, Hit(Note(notes[0]), duration + math.sin(time)))
  
  if i < iterations - 2 and i % skip == 0:
    timeline.add(time, Hit(Note(notes[2]).shift_down_octave(1), duration + math.sin(time)))
